...
**Submission:** 25-11-2025  
**Marks:** 5

---

## Exercise 14: Project Overview
...
_Path: EC2 → Launch Templates → Create launch template_

- **Template Name:** `LT-WebServer`
    
- **Application and OS Images (AMI):** * Click **My AMIs** * Select your custom AMI: `AMI-WebServer`
...
> **Result:** The Launch Template is now ready for the Auto Scaling Group.

---

## STEP B: Create Auto Scaling Group (ASG)

_Path: EC2 → Auto Scaling Groups → Create Auto Scaling Group_

1. **Name & Template:** * Name: `ASG-WebServer`
...
5. **Review & Create:** Verify settings and click **Create Auto Scaling Group**.
...
_Path: CloudWatch → Alarms → All alarms → Create alarm_
...
On Configure actions page, do NOT add anything. Just click Next. If a popup says _“No actions configured”_, click Proceed without actions.
...
_Path: EC2 → Auto Scaling groups → ASG-WebServer → Automatic scaling_
...
_Path: CloudWatch → Alarms → Create alarm_
...
**Note:** Ensure your VPC has internet access (e.g., via NAT Gateway) for `dnf install` to work.
...
Copy the **ALB DNS Name** (Found in _EC2 → Load Balancers_) and paste it into your browser. Refresh several times to see traffic shifting between instances:
...
**What is a Target Group?** A list of servers (EC2 instances) behind the load balancer. It defines the port (80) and health check rules.

**Why two steps (Launch Template vs ASG)?**

- **Launch Template:** The "Blueprint" (WHAT to launch: AMI, type, SG).
    
- **ASG:** The "Manager" (WHEN and HOW MANY to launch).
    

**What is ALB DNS?** The public URL for your application (e.g., `mywebapp-alb-123.ap-south-1.elb.amazonaws.com`). It acts as the entry point for users.
...
## Clean-Up Procedure (Required)

**Important:** Delete resources in this exact order to avoid dependency errors (e.g., ALB cannot be deleted if ASG is attached).
