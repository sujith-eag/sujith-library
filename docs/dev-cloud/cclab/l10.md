...
graph TB
    subgraph "AWS VPC (10.0.0.0/16)"
        ...
        web -- "Internal Traffic (via Private IP)" --> db
        db -- "Outbound for Updates" --> nat
        nat -- "Forwards to IGW" --> igw
    end

    internet("Internet User")
    internet -- "HTTP/S Request" --> igw
    igw -- "Forwards to Web Server" --> web
```

## 4. Advanced Mermaid Styling Demo



This section demonstrates advanced styling techniques to create professional and clear diagrams.



### a) Comprehensive VPC Architecture Diagram



This diagram uses styled subgraphs, nodes, and links to provide a clear and detailed overview of the VPC architecture.
...
graph TD
    ...
    classDef publicSubnet fill:#e6f7ff,stroke:#91d5ff,stroke-width:2px;

    classDef privateSubnet fill:#fafafa,stroke:#d9d9d9,stroke-width:2px;

    classDef gateway fill:#d4edda,stroke:#155724,stroke-width:2px;

    classDef instance fill:#fffbe6,stroke:#ffe58f,stroke-width:2px;
    ...
    subgraph "AWS Cloud"
        ...
        subgraph "VPC (10.0.0.0/16)"
            ...
            subgraph "Public Subnet (10.0.1.0/24)"
                ...
                style PublicSubnet fill:#e6f7ff,stroke:#91d5ff

                A[fa:fa-server Web Server]:::instance

                B[fa:fa-random NAT Gateway]:::gateway

            end

            subgraph "Private Subnet (10.0.2.0/24)"

                style PrivateSubnet fill:#fafafa,stroke:#d9d9d9

                C[fa:fa-database Database]:::instance

            end

            IGW[fa:fa-globe Internet Gateway]:::gateway
    ...
    User[fa:fa-user User] -->|HTTPS| IGW
...
This sequence diagram uses `themeVariables` to apply custom colors to the participants.
...
sequenceDiagram
    ...
    %%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#a7e9ff', 'primaryTextColor': '#333', 'lineColor': '#87CEEB'}}}%%
...
### c) Styled Class Diagram



This class diagram uses `classDef` to apply different colors to interfaces and classes.
...
classDiagram
    ...
    class AwsResource:::interface

    class VPC:::component

    class Subnet:::component

    class EC2Instance:::component
...
This chart classifies VPC components based on their function and scope.
...
quadrantChart
    ...
    title VPC Component Classification

    x-axis "Internal" --> "External-Facing"

    y-axis "Network-Level" --> "Instance-Level"

    quadrant-1 "High-Level Control"

    quadrant-2 "Instance Security"

    quadrant-3 "Subnet Security"

    quadrant-4 "Core Networking"

    "Route Tables": [0.2, 0.8]

    "Security Groups": [0.8, 0.8]

    "Network ACLs": [0.2, 0.2]

    "Internet Gateway": [0.8, 0.2]
