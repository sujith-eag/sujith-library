...
1. **Step 1:** User requests data.
    
2. **Step 2:** Application checks ElastiCache (Redis).
    
3. **Step 3:** If data exists $\rightarrow$ return from cache (**FAST**).
    
4. **Step 4:** If data does not exist $\rightarrow$ fetch from RDS.
    
5. **Step 5:** Store fetched data in ElastiCache.
    
6. **Step 6:** Return data to user.
...
1. **Select Region:** Choose **ap-south-1 (Mumbai)**. Ensure EC2 and ElastiCache stay in the same region.
...
3. **Network & Security Group:**
    ...
    - Inbound Rules:
        ...
        | SSH | 22 | 0.0.0.0/0 |
	Do NOT add Redis (6379) here (The client does not listen on 6379) Outbound rules: Allow all (default)
	
1. Connect & Install Client:
    ...
    _Note: Amazon Linux 2023 uses **Valkey**, which is fully Redis-protocol compatible._
...
- **Creation method:** Easy create
    ...
    - _Note: Easy create is used to avoid advanced production settings._
...
- **Subnet Group:** Select **Create a new subnet group**
    ...
    - **Subnets:** Leave AWS auto-selected subnets unchanged (No manual subnet selection required).
...
- _Note: This allows only the EC2 client to access Redis._
...
> **Note:** We have created an in-memory Redis cache using Amazon ElastiCache. It runs inside the AWS VPC and does not have a public IP. Only our EC2 client is allowed to access it using port 6379.
...
- _Note: This confirms the EC2 is ready to act as a Redis-compatible client._
...
- **4.2 Store and Retrieve Data (SET / GET)**
    ...
    - Command: `SET course "Cloud Computing"`
        
    - Command: `GET course`
        
    - Expected Output: `"Cloud Computing"` (Demonstrates key–value storage in memory).
...
- **4.3 Counter Example (INCR)**
    ...
    - Expected Output: `"2"` (Note: Common real-world use - page views, hit counters).
        
- **4.4 Set Data with Expiry (TTL)**
    
    - Command: `SET notice "Results Published" EX 60`
        
    - Command: `TTL notice`
        
    - Command: `GET notice`
        
    - Expected Output: **TTL shows a value ≤ 60**; **GET returns "Results Published"**.
        
- **4.5 Verify Automatic Expiry**
    
    - Wait ~60 seconds, then run: `GET notice`
        
    - Expected Output: `(nil)` (Confirms data is automatically removed from memory).
...
> **Note:** "The application first checks Redis. If data is present, it is returned immediately from memory. If not present, the application fetches data from the database and stores it in Redis with a TTL. Redis automatically removes the data after expiry."
...
- **Step 1: Delete Redis Cluster:** ElastiCache → Select `lab-redis` → **Delete** (Disable snapshots).
    
- **Step 2: Terminate EC2 Instance:** EC2 → Instances → **Terminate** `RedisClient-EC2`.
    
- **Step 3: Optional:** Delete unused security groups.
...
_The above commands simulate application caching behavior. Cached data is temporary and stored in memory. After expiry, the application would fetch fresh data from the database again._
...
RDS instance to test the full "database + cache" workflow?