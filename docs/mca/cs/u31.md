...
A widely used model to understand the stages of a cyberattack is the **Cyber Kill Chain**. This framework outlines the typical steps an attacker takes to compromise a network and achieve their objective.

#### 1. Reconnaissance
    
The attacker gathers information about the target. This can involve scanning for open ports, identifying employee emails, or finding details about the target's network infrastructure and software versions.
    
#### 2. Weaponization
    
The attacker creates a malicious payload (like a virus or ransomware) and pairs it with an exploit (a piece of code that takes advantage of a vulnerability). This could be a malicious PDF or a Word document with a macro.
    
#### 3. Delivery
    
The weaponized payload is sent to the target. Common delivery methods include phishing emails, malicious links on websites, or infected USB drives.
    
#### 4. Exploitation
    
The malicious code is triggered. This happens when the victim opens the infected attachment or clicks the malicious link, causing the exploit to run and take advantage of a software vulnerability.
    
#### 5. Installation
    
The malware installs itself on the victim's system, establishing a foothold. This often involves creating a backdoor to allow the attacker persistent access.
    
#### 6. Command & Control (C2)
    
The malware "phones home" to a C2 server controlled by the attacker. This allows the attacker to remotely control the compromised system, issue commands, and exfiltrate data.
    
#### 7. Actions on Objectives
    
The attacker carries out their final goal. This could be stealing sensitive data (data exfiltration), encrypting files for ransom, destroying systems, or using the compromised machine to attack other targets.
