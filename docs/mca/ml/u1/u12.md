...
| **Category**                                        | **Primary Goal**                                                                             | **Learning Mechanism**                                           |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| **Supervised Learning**<br>(Predictive Learning)    | **Prediction** of a class or value for unknown data based on prior labeled data.             | Learning from **labeled** training data (guided by a "teacher"). |
| **Unsupervised Learning**<br>(Descriptive Learning) | **Discovery** of underlying groups, patterns, or structures in unlabeled data.               | Learning from **unlabeled** data (self-discovery).               |
| **Reinforcement Learning**<br>(Agent Learning)      | **Action** to achieve a goal in an environment, guided by a system of rewards and penalties. | Learning by self (trial and error).                              |

## Supervised Learning

Supervised learning (SL) is a type of machine learning defined by its requirement for **labeled training data** which serves as the **experience ($E$)** on a specific task a machine has to execute. (Experience as Labeled Data) 
...
Classification is used when the output variable is **categorical or nominal** (i.e., it belongs to a finite set of discrete classes or labels). Objective is to assign a label, category, or class to a new, unseen instance.

Classification is a type of supervised learning where a target feature, which is of type categorical (Nominal / Discrete), is predicted for test data based on the information imparted by training data. The target categorical feature is known as class.
...
- A powerful classifier that finds a **separating hyperplane** (a decision boundary) in a high-dimensional space.
...
- Uses **kernels** (e.g., linear, polynomial, RBF) to transform non-linearly separable data into a higher dimension where a linear separator can be found.
...
The process of partitioning data to train and validate models.
