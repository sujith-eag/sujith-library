...
Footprinting (passive information gathering) and reconnaissance (active probing) are typically the first phases of an engagement. These phases collect intelligence about a target to identify potential vulnerabilities without (passive) or with (active) interaction with the target systems.

Topics Covered
- What is footprinting and reconnaissance?
- Web searches and Google hacks (Google Dorking)
- WHOIS records and name servers
...
- It relies on Open Source Intelligence (OSINT) such as search engines, public records, social media, and third‑party services. Passive collection can reveal potential vulnerabilities and weaknesses.

## Reconnaissance (Active)

- Reconnaissance (active footprinting) uses scanning and probing to discover live hosts, open ports, services, and versions. This is more intrusive than passive footprinting.

- Common tools: `nmap`, `masscan`, and service-specific probes.


Keeping Inventory
- Maintain an organized inventory (spreadsheet or database) to track findings: names, dates, links, IP addresses, OS, exposed services, and login endpoints.
...
| -------------------------- | -------------------------------------- | -------------------------------- |
| `site:`                    | Search within a specific domain        | `site:wiley.com cybersecurity`   |
| `filetype:` / `ext:`       | Find specific file types or extensions | `filetype:pdf "ethical hacking"` |
| `intitle:` / `allintitle:` | Pages with words in the title          | `intitle:"login page"`           |
| `inurl:` / `allinurl:`     | Words in the URL                       | `inurl:admin`                    |
| `intext:` / `allintext:`   | Words in page body                     | `intext:"confidential"`          |
| `cache:`                   | View Google's cached version           | `cache:example.com`              |
| `related:`                 | Find related sites                     | `related:cnn.com`                |
| `link:`                    | Pages that link to a domain            | `link:starbucks.com`             |
...
### Google Dorks / History

- The concept of Google Dorking was popularized by Johnny Long (~2002). Many queries are collected in the Google Hacking Database (GHDB)
...
- Useful tools: `nslookup`, `dig`, and online DNS lookup services to query A, MX, TXT, CNAME, and other records.
...
- In the US, consult the SEC EDGAR database for company filings (10‑K, annual reports): `https://www.sec.gov/edgar/searchedgar/companysearch.html`.
...
- Validate collected information—data can be outdated, incorrect, or intentionally misleading.
...
- LinkedIn is especially useful for discovering employee names, roles, and technologies—valuable for targeted social‑engineering.
...
- Recommended tool: ExifTool — `https://exiftool.org/` — it extracts extensive metadata from images and documents.
...
- Maltego : is a visual tool that takes data points and, by using what they call transforms, tries to collect and/or connect data to draw a bigger picture. visual link analysis for aggregating relationships between domains, people, and infrastructure.
...
## OSINT Tools: Maltego, GitHub Monitoring, SpiderFoot, Dmitry, Shodan, Archive.org

### Maltego

- Maltego is a visual link-analysis tool that ingests data points and uses "transforms" to discover and connect entities (domains, people, IPs, emails, infrastructure).
...
- Maltego offers a free Community edition for basic use.
- Website: https://www.maltego.com/maltego-community/ (Community/free edition available).


### GitHub and Online Forums

- GitHub, forums, and social platforms (X, Reddit, StackOverflow, Glassdoor) frequently leak sensitive information — secrets, API tokens, credentials, internal docs, or developer discussions.
- Monitor and search these platforms to detect exposed secrets and project information. Example incident: leaked Mercedes source code reported at SecurityWeek: https://www.securityweek.com/leaked-github-token-exposed-mercedes-source-code/
...
- GitHub guidance: https://docs.github.com/en/code-security/getting-started/best-practices-for-preventing-data-leaks-in-your-organization
- Monitoring services: GitGuardian — https://www.gitguardian.com/

- Additionally, other service organizations have also recognized this and offer monitoring services to organizations to help mitigate the problem. One such organization is GitGuardian, https://www. gitguardian.com/, which will monitor GitHub for you.
...
- Offers a free tier and is included in many pentest distributions (e.g., Kali). Website: https://www.spiderfoot.net/
...
- Shodan is an internet-wide search engine that indexes devices, services, and their exposed ports and banners. It can reveal devices and services already scanned and categorized.
- It crawls the internet, cataloging devices and vulnerabilities.
...
- Combine multiple tools for richer context: for example, use Maltego/SpiderFoot to expand entity lists, GitHub/Forum searches to find leaked assets, Dmitry for quick enumeration, and Shodan/Archive.org for historical and exposed devices.
- Export and save results (CSV/XML) and record findings in your inventory for correlation with WHOIS, EXIF, and other OSINT data.
