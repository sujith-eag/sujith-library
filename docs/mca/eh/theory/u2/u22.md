...
After the footprinting phase, you will have gathered general information about the target. The scanning and enumeration phase digs deeper to collect finer details such as active hosts, open ports, running services, and network topology.
...
### Common TCP Scan Types
  ...
  * **TCP Connect scan (`-sT`):** Completes the full TCP handshake. Reliable but noisy and easily detected.
  * **TCP SYN scan (`-sS`):** "Half-open" or stealth scan. Sends SYN and interprets SYN/ACK (open) or RST (closed) without sending the final ACK. Harder to log.
  * **TCP FIN scan:** Sends a FIN packet. Useful on some Unix systems (closed ports return RST).
  * **TCP NULL scan:** No flags set. Illegal packet that can elicit specific responses from different OS stacks.
  * **TCP ACK scan:** Sends packets with the ACK flag to map firewall rules and ACLs.
...
Use `-T0` to `-T5` to adjust general timing templates:

  * **`-T0` (Paranoid):** Very slow (every 5 mins), serial processing. Evades IDS.
  * **`-T1` (Sneaky):** Slow (every 15s).
  * **`-T2` (Polite):** Gentle (0.4s wait). Good for fragile networks.
  * **`-T3` (Normal):** Default. Parallel scanning enabled.
  * **`-T4` (Aggressive):** Fast (1.25s wait). Recommended for modern, reliable networks.
...
1.  **Plan Scans:** Inform stakeholders to avoid accidental outages or excess alerts.
2.  **Start Slow:** Begin with discovery and light scans, then increase intensity.
3.  **Use Output Files:** Always save results (`-oX`, `-oA`) for inventory and reporting.
4.  **Test Scripts:** Test NSE scripts in lab environments before running them in production to prevent unintended crashes.
