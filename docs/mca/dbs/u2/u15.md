...
### Table: `dept_bn` – Department Details
...
### Table: `salary_grade_bn` – Salary Grades
...
Add a **foreign key constraint** to ensure that `emp_bn.dept_id` references a valid department in `dept_bn`.
...
> This enforces **referential integrity** between the `emp_bn` and `dept_bn` tables.
...
> Dates are explicitly cast using `DATE 'YYYY-MM-DD'` for better portability and type safety.

## Key Notes

- Each **employee** belongs to one **department**, linked by `dept_id`.
    
- The `manager_id` refers to another employee (self-referencing foreign key could be added for data integrity).
    
- The `commission` field is `NULL` for employees who don’t receive commission.
    
- The `salary_grade_bn` table is useful for grading employees based on salary ranges.
    
- Foreign key constraints ensure **data consistency** between employees and departments.
